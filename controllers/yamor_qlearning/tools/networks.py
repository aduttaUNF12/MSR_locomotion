...
class CNN(nn.Module):
    def __init__(self, number_of_modules, lr, n_actions):
        ...
        # 2d [32, 12, 3, 3]
        # 3d [32, 12, 3, 3, 3]
        self.conv1 = nn.Conv1d(in_channels=4*number_of_modules, out_channels=32, kernel_size=3).to(self.device)
        ...
        self.conv2 = nn.Conv1d(in_channels=32, out_channels=64, kernel_size=3).to(self.device)
        self.bn2 = nn.BatchNorm1d(64, affine=False).to(self.device)

        self.conv3 = nn.Conv1d(in_channels=64, out_channels=128, kernel_size=3).to(self.device)
        ...
        # x = torch.tensor([0., 0., 1., 1., 0., 0., 1., 0., 0., 0., 1., 0.], dtype=torch.float32).to(self.device)
